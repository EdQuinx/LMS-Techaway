...
namespace LibraryManagementSystem.UI.Book_Forms
{
    public partial class frmAddBook : WeifenLuo.WinFormsUI.DockContent
    {
        ...
        private void BtnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                ep.SetError(txtBookName, "Book Name cannot be left blank!");
                txtBookName.Focus();
                return;
            }
            if (txtTitle.Text.Trim().Length == 0)
            {
                ep.SetError(txtTitle, "Title cannot be left blank!");
                txtTitle.Focus();
                return;
            }
            if (txtAuthor.Text.Trim().Length == 0)
            {
                ep.SetError(txtAuthor, "Author cannot be left blank!");
                txtAuthor.Focus();
                return;
            }
            if (txtEdition.Text.Trim().Length == 0)
            {
                ep.SetError(txtEdition, "Edition cannot be left blank!");
                txtEdition.Focus();
                return;
            }
            if (txtNoOfCopies.Text.Trim().Length == 0)
            {
                ep.SetError(txtNoOfCopies, "Edition cannot be left blank!");
                txtNoOfCopies.Focus();
                return;
            }
            if (txtPrice.Text.Trim().Length == 0)
            {
                ep.SetError(txtPrice, "Price cannot be left blank!");
                txtPrice.Focus();
                return;
            }
            ...
            if (!BooksHelper.HaveBook(txtBookName.Text, txtAuthor.Text, txtAuthor.Text))
            {
                ...
                MessageBox.Show("Add Book Successful", "Library Management System", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                MessageBox.Show("The book you want to add already exists in the system!", "Library Management System", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
