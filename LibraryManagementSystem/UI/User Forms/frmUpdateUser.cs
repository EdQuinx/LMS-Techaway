...
namespace LibraryManagementSystem.UI.User_Forms
{
    public partial class frmUpdateUser : WeifenLuo.WinFormsUI.DockContent
    {
        ...
        private void BtnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                ep.SetError(txtSearchUserName, "Username cannot be left blank!");
                txtSearchUserName.Focus();
                return;
            }
            if (txtSearchPassword.Text.Trim().Length == 0)
            {
                ep.SetError(txtSearchPassword, "Password cannot be left blank!");
        ...
        private void BtnUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                ep.SetError(txtUserName, "Username cannot be left blank!");
                txtUserName.Focus();
                return;
            }
            if (txtNewPassword.Text.Trim().Length == 0)
            {
                ep.SetError(txtNewPassword, "New Password cannot be left blank!");
            ...
            if (txtNewPassword.Text.Trim() == txtConfirmPassword.Text.Trim())
            {
                ...
                if (!UsersHelper.HaveUserID(txtUserName.Text.ToLower(), userid))
                {
                    ...
                    MessageBox.Show("Member Update Successful", "Library Management System", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                ep.SetError(txtConfirmPassword, "Password and Confirm Password do not match!");
