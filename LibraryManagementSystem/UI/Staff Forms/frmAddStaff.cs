...
namespace LibraryManagementSystem.UI.Staff_Forms
{
    public partial class frmAddStaff : WeifenLuo.WinFormsUI.DockContent
    {
        ...
        {
            epName.Clear();
            bool check1 = StaffsHelper.ControlValidate(txtName, "Please Enter Staff Name.", epName);

            epTCNO.Clear();
            bool check2 = StaffsHelper.ControlValidate(txtTcNo, "Please Enter TC Number.", epTCNO);

            epAddress.Clear();
            bool check3 = StaffsHelper.ControlValidate(txtAddress, "Please Enter Address.", epAddress);

            epContactNo.Clear();
            bool check4 = StaffsHelper.ControlValidate(txtContactNo, "Please Enter Contact Number.", epContactNo);

            if (txtTcNo.Text.Trim().Length != 11)
            {
                MessageBox.Show("TC No should have 11 digits.", "Library Management System", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (check1 && check2 && check3 && check4)
            {
                if (!StaffsHelper.HaveTCNO(txtTcNo.Text.Trim()) && !StaffsHelper.HaveContactNo(txtContactNo.Text))
                {
                    ...
                    MessageBox.Show("Adding Staff Successful", "Library Management System", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    FillGrid();
                }
                else
                {
                    MessageBox.Show("The TC or Telephone number you entered is registered in the system.", "Library Management System", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvStaffList.Rows.Count > 0)
                {
                    ...
                    DialogResult dialogResult = MessageBox.Show("Are you sure you want to delete Staff?", "Library Management System", MessageBoxButtons.YesNo);
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        MessageBox.Show("Staff Delete Successful", "Library Management System", MessageBoxButtons.OK, MessageBoxIcon.Information);
